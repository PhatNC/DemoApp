...
async function downloadFile(urlFile) {
    const url = urlFile;
    const fpath = path.resolve(__dirname, '../downloads', 'tempVideo.txt')
...
async function showResult(auth) {
    ...
    fs.readFile('./downloads/tempVideo.txt', function (err, data) {
        if (err) throw err;
...
async function uploadFile(auth) {
    ...
    if (fileUploaded == undefined) {
        tempRes.redirect('/video?connect=' + isDone.toString());
        return;
    }
    else {
        ...
        if (type != '' && model_name != '') {
            ...
            client.connect(50000, '171.244.21.155', () => {
                ...
                console.log('Send');
                console.log('Link Post ', postData);
            ...
            client.on('data', (data) => {
                ...
                vidLink = linkOutput[0].replace('&export=download', '');
                console.log('Link 1' + vidLink);
                fileID = vidLink.split('id=')[1];
                client.destroy();
                client.end();
                tempRes.redirect('/video/show?vidLink=' + vidLink);
...
router.post('/uploadVideo', multer(multerConfig).single('file_upload'), function (req, res) {
